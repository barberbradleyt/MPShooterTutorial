...
void ABlasterPlayerController::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ABlasterPlayerController, bShowTeamsScores);
}
...
void ABlasterPlayerController::OnRep_ShowTeamsScores()
{
	if (bShowTeamsScores)
	{
		InitTeamScores();
...
void ABlasterPlayerController::ServerCheckMatchState_Implementation()
{
	...
	if (GameMode)
	{
		...
		bShowTeamsScores = GameMode->bTeamsMatch;

		ClientJoinMidgame(MatchState, WarmupTime, MatchTime, CooldownTime, LevelStartingTime, bShowTeamsScores);
	...
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				...
				{
					//if (bInitializeDefeats) SetHUDGrenades(BlasterCharacter->GetCombat()->GetGrenades());
	...
	if (BlasterHUD)
	{
		...
		if (bHUDValid)
		{
			...
			if (BlasterGameState)
			{
				...
				FString InfoTextString = bShowTeamsScores ? GetTeamsInfoText(BlasterGameState) : GetInfoText(TopPlayers);
